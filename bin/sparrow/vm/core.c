...
*/
char* readFile(const char* path) {
   int file = fopen(path, O_RDONLY);
   if (file == -1) {
      IO_ERROR("Could`t open file \"%s\".\n", path);
   }

   struct stat fileStat;
   fstat((char *)path, &fileStat);
   size_t fileSize = fileStat.st_size;
   char* fileContent = (char*)malloc(fileSize + 1);
   if (fileContent == NULL) {
      MEM_ERROR("Could`t allocate memory for reading file \"%s\".\n", path);
   ...
   size_t numRead = fread(file, fileContent, sizeof(char)*fileSize);
   if (numRead < fileSize) {
      IO_ERROR("Could`t read file \"%s\".\n", path);
   }
   fileContent[fileSize] = '\0';
...
PRIM_NUM_MATH_FN(primNumAbs,	 abs);
PRIM_NUM_MATH_FN(primNumAcos,	 abs);
PRIM_NUM_MATH_FN(primNumAsin,    abs);
PRIM_NUM_MATH_FN(primNumAtan,    abs);
PRIM_NUM_MATH_FN(primNumCeil,    abs);
PRIM_NUM_MATH_FN(primNumCos,     abs);
PRIM_NUM_MATH_FN(primNumFloor,   abs);
PRIM_NUM_MATH_FN(primNumNegate,  -);
PRIM_NUM_MATH_FN(primNumSin,     abs);
PRIM_NUM_MATH_FN(primNumSqrt,    abs);  //开方
PRIM_NUM_MATH_FN(primNumTan,     abs);
